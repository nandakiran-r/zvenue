...
function getStatusBadge(status: string) {
  switch (status) {
    ...
    case 'pre_booked':
      return <Badge className="bg-orange-100 text-orange-700 hover:bg-orange-100">Pre-Booked</Badge>
    case 'pending':
        ...
        <Card><CardContent className='p-0'>
          <Table>
            <TableHeader><TableRow>
              ...
              <TableHead className='text-center'>Reg. Fee</TableHead>
              <TableHead className='text-center'>Balance</TableHead>
              <TableHead className='text-center'>Status</TableHead>
            </TableRow></TableHeader>
            <TableBody>
              {isLoading ? Array.from({length:4}).map((_,i)=><TableRow key={i}>{Array.from({length:9}).map((_,j)=><TableCell key={j}><Skeleton className='h-4 w-20'/></TableCell>)}</TableRow>) :
              (bookings||[]).map((b: any) => (
                <TableRow key={b.id}>
                  <TableCell>
                    <div className='flex items-center gap-2'>
                      <Avatar className='h-8 w-8'>
                        ...
                        <AvatarFallback>{(b.user?.full_name||'U')[0]}</AvatarFallback>
                      </Avatar>
                      <div>
                        <p className='text-sm font-medium'>{b.user?.full_name||'Unknown'}</p>
                        <p className='text-xs text-muted-foreground'>{b.user?.email}</p>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <p className='text-sm font-medium'>{b.venue?.name}</p>
                    <p className='text-xs text-muted-foreground flex items-center gap-1'><MapPin className='h-3 w-3'/>{b.venue?.city}</p>
                  </TableCell>
                  ...
                  <TableCell className='text-center text-sm'>
                    {b.registration_fee_paid > 0 ? (
                      <span className='text-green-600 font-medium'>₹{b.registration_fee_paid.toLocaleString('en-IN')}</span>
                    ) : '—'}
                  </TableCell>
                  <TableCell className='text-center text-sm'>
                    {b.remaining_balance > 0 ? (
                      <span className='text-orange-600 font-medium'>₹{b.remaining_balance.toLocaleString('en-IN')}</span>
                    ) : '—'}
                  </TableCell>
                  <TableCell className='text-center'>{getStatusBadge(b.status)}</TableCell>
                </TableRow>
              ))}
              {!isLoading && (bookings||[]).length===0 && <TableRow><TableCell colSpan={9} className='h-24 text-center text-muted-foreground'>No bookings yet</TableCell></TableRow>}
            </TableBody>
          </Table>
        </CardContent></Card>
      </Main>
